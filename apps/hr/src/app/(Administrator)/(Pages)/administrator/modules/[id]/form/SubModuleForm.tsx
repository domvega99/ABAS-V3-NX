...
import { showSubModuleById, SubModule } from '@/app/services/submodule';
...
interface Props {
    title: string;
    onAdd?: (newSubModule: SubModule, setErrorFields: React.Dispatch<React.SetStateAction<any>>) => Promise<any>;
    onUpdate?: (updatedRole: SubModule, id: number, setErrorFields: React.Dispatch<React.SetStateAction<any>>) => Promise<any>;
    id?: number;
    module_id?: number;
}

const SubModuleForm = ({ title, onAdd, onUpdate, id, module_id }: Props) => {
    ...
    const handleSave = async () => {
      try {
        ...
        if (onAdd) {
            const { status, message } = await onAdd(subModuleData, setErrorFields);
            if (status === 'success') {
                setSuccess(message);
            ...
            if (onUpdate && id) {
                const { status, message } = await onUpdate(subModuleData, id, setErrorFields);
                if (status === 'success') {
                    setSuccess(message);
                    setErrorFields({});
                    setError('');
